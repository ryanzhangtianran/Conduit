...
ConduitLoadingHandle showConduitLoadingModal(
  ...
  final overlay = AppOverlayRegistry.overlayKey?.currentState;
...
/// One option of [showConduitChoiceDialog].
class ConduitChoice<T> {
  const ConduitChoice(this.value, this.label, {this.hint});

  final T value;
  final String label;

  /// Explains the option; shown under the picker while it is selected.
  final String? hint;
}

/// Asks the user to pick one of [choices] from the app's dropdown and
/// confirm. Returns the picked value, or null when cancelled.
Future<T?> showConduitChoiceDialog<T>(
  BuildContext context, {
  required String title,
  required List<ConduitChoice<T>> choices,
  required T initial,
  String? confirmLabel,
}) {
            ...
            child: ConduitDropdown<T>(
              ...
              entries: [
                for (final choice in choices)
                  DropdownMenuEntry(value: choice.value, label: choice.label),
              ],
              onChanged: (value) {

...
class _ActivityTabState extends ConsumerState<ActivityTab> {
  ...
  bool get _hasGpu => _history.any((sample) => sample.gpuPercent != null);
  ...
  Future<void> _poll() async {
    ...
    _loading = true;
    try {
      final counters = await _collectSshCounters();
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                  ...
                  child: SegmentedButton<_ActivityChart>(
                    ...
                    segments: [
                      for (final chart in _ActivityChart.values)
                        if (chart != _ActivityChart.gpu || _hasGpu)
                          ButtonSegment(
                            value: chart,
                ...
                const SizedBox(height: 12),
                Expanded(child: _buildChart(latest, scheme)),
              ],
            ),
          ),
        ),
  ...
  Widget _buildChart(
    ...
    ColorScheme scheme,
  ) => switch (_chart == _ActivityChart.gpu && !_hasGpu
      ? _ActivityChart.cpu
      : _chart) {
  ...
  String _gpuSubtitle(ActivitySample s) {
    ...
    final memory = s.gpuMemoryUsedKb == null || s.gpuMemoryTotalKb == null
        ? ''
        : ' · ${_formatKb(s.gpuMemoryUsedKb!)} / ${_formatKb(s.gpuMemoryTotalKb!)}';

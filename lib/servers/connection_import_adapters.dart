...
class ImportedConnection {
  ...
  /// Display name of the saved credential; defaults to the server name.
  final String? credentialName;

  /// Position of [credential] in the export's `savedCredentials` list when
  /// the source shared one credential row between servers. Connections with
  /// the same index are linked to a single re-created row on import.
  final int? sharedCredentialIndex;
...
/// The only third-party format the importer reads.
///
...
class OpenSshConfigAdapter {
  const OpenSshConfigAdapter();

  /// Origin label shown in the import preview.
  String get name => 'OpenSSH config';

  /// Whether [content] looks like an OpenSSH config. Never throws.

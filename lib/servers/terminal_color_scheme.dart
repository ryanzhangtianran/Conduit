...
/// A terminal palette as edited in Settings and persisted per brightness.
///
/// The ANSI order follows the conventional xterm layout: normal colors first,
/// then their bright counterparts. [toTerminalTheme] is the single bridge to
/// flterm's renderer theme; [TerminalColorScheme.fromTerminalTheme] inverts
/// it.
@immutable
class TerminalColorScheme {
  ...
  /// Rebuilds a scheme from the colour part of a flterm theme.
  factory TerminalColorScheme.fromTerminalTheme(
    flterm.TerminalTheme theme, {
    required String id,
    ...
    cursor: theme.cursor.color?.fixedColor ?? theme.palette.foreground,
    selection:
        theme.selection.background?.fixedColor ?? theme.palette.foreground,
    ansiColors: theme.palette.ansiColors,
  );

  /// The renderer theme for this palette. Font and cursor-motion settings are
  /// layered on by the caller with `copyWith`.
  flterm.TerminalTheme toTerminalTheme() => flterm.TerminalTheme(
    palette: flterm.ColorPalette(
      ansiColors: ansiColors,
      // Keep an opaque palette color for libghostty's color resolution.
      // Transparency belongs to TerminalTheme.backgroundOpacity; passing an
      // alpha-zero palette color is flattened to black by the renderer.
      background: background,
      foreground: foreground,
    ),
    cursor: flterm.CursorTheme(color: flterm.DynamicColor.fixed(cursor)),
    selection: flterm.SelectionTheme(
      background: flterm.DynamicColor.fixed(selection),
    ),
    backgroundOpacity: 1,
  );
  ...
  bool operator ==(Object other) =>
      ...
      other is TerminalColorScheme &&
          ...
          listEquals(other.ansiColors, ansiColors);

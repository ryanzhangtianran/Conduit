...
class Socks5ServerHandshake {
  Socks5ServerHandshake(Stream<List<int>> input, this._output)
    : _reader = BufferedSocketReader(
        input,
        closedError: () => const Socks5ProtocolException(
          'The client closed the connection during the SOCKS5 handshake.',
        ),
      );

  final IOSink _output;
  final BufferedSocketReader _reader;

  Future<Uint8List> _read(int count, Duration timeout) =>
      _reader.read(count, timeout);
  ...
  void startPump() => _reader.startPump();

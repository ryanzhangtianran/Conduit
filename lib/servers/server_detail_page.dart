...
import 'server_processes_provider.dart';
import 'server_providers.dart';

/// The Monitor page's server workspace; always hosted by [MonitorPage].
///
/// Watching [focusedServerProcessesProvider] drives the refresh loop and
/// marks the server as focused for the background scheduler; nothing here
/// keeps a timer or a copy of the list.
class ServerDetailPage extends ConsumerWidget {
  const ServerDetailPage({super.key, required this.server, this.header});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final session = sessions
        ...
        .firstOrNull;
    final processes = ref.watch(focusedServerProcessesProvider(server.id));

    Future<void> refreshProcesses() async =>
        ref.invalidate(focusedServerProcessesProvider(server.id));

    return _DetailWorkspace(
      server: server,
      session: session,
      connected: session?.status == SessionStatus.connected,
      processes: processes,
      refreshInterval: ref.watch(focusedServerRefreshIntervalProvider),
      onConnect: () async {
        final connected = await connectForStatistics(context, ref, server);
        if (connected) await refreshProcesses();
      },
    ...
    final specs = [
      ...
      _SpecItem(
        ...
        value: stats?.memoryTotalKb == null
            ? '—'
            : formatKilobytes(stats!.memoryTotalKb!),
      ),
      _SpecItem(
        ...
        value: stats?.diskTotalKb == null
            ? '—'
            : formatKilobytes(stats!.diskTotalKb!),
      ),
      _SpecItem(
    ...
    return Column(
      children: [
          ...
          _MetricCard(
            ...
            detail: [
              ...
              if (gpu.memoryUsedKb != null && gpu.memoryTotalKb != null)
                '${formatKilobytes(gpu.memoryUsedKb!)} / ${formatKilobytes(gpu.memoryTotalKb!)}',
        ...
        _MetricCard(
          icon: Symbols.memory,
          label: 'detailMemory',
          value: memoryUsed == null ? '—' : formatKilobytes(memoryUsed),
          detail: stats!.memoryTotalKb == null
              ? null
              : 'detailOf'.tr(args: [formatKilobytes(stats!.memoryTotalKb!)]),
        ...
        _MetricCard(
          icon: Symbols.storage,
          label: 'detailRootDisk',
          value: diskUsed == null ? '—' : formatKilobytes(diskUsed),
          detail: stats!.diskTotalKb == null
              ? null
              : 'detailOf'.tr(args: [formatKilobytes(stats!.diskTotalKb!)]),
        ...
        _MetricCard(
          ...
          detail: swapUsed == null || stats!.swapTotalKb == null
              ...
              : 'detailSwap'.tr(
                  args: [
                    formatKilobytes(swapUsed),
                    formatKilobytes(stats!.swapTotalKb!),
                  ],
                ),
        ),
      ],
...
class _ProcessListState extends ConsumerState<_ProcessList> {
  ...
  Future<void> _killProcess(ServerProcess process) async {
    ...
    try {
      final sudoPassword = await sudoPasswordFor(ref, widget.server);
      await ref

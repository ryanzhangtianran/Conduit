...
import 'package:conduit/shared/presentation/deploy_terminal.dart';
import 'package:conduit/shared/presentation/app_scaffold.dart';
import 'package:styled_widget/styled_widget.dart';
import 'server_providers.dart';
import 'terminal_tabs_provider.dart';

/// The six reorderable workspace tabs, indexed by their route position in
/// [ServerWorkspacePage]'s tab router. Settings (route index 6) stays fixed
/// at the bottom of the rail.
class _TabSpec {
  const _TabSpec(this.icon, this.labelKey, {this.badged = false});
...
const _tabSpecs = [
  _TabSpec(Symbols.dashboard, 'tabDashboard'),
  _TabSpec(Symbols.terminal, 'tabTerminal'),
  _TabSpec(Symbols.monitoring, 'tabMonitor'),
  _TabSpec(Symbols.dns, 'assetsConnections'),
  _TabSpec(Symbols.rocket_launch, 'tabGithub', badged: true),
  _TabSpec(Symbols.swap_horiz, 'tabPortForwarding'),
];
              ...
              : Material(
                  ...
                  child: NavigationBar(
                    ...
                    onDestinationSelected: (index) => tabsRouter.setActiveIndex(
                      index == order.length ? 6 : order[index],
                    ),
                    destinations: [
                      for (final routeIndex in order)
                        NavigationDestination(
                          icon: Badge(
                            isLabelVisible:
                                _tabSpecs[routeIndex].badged &&
                                githubHasFailures,
                            child: Icon(_tabSpecs[routeIndex].icon),
                          ),
                          selectedIcon: Badge(
                            isLabelVisible:
                                _tabSpecs[routeIndex].badged &&
                                githubHasFailures,
                            child: Icon(_tabSpecs[routeIndex].icon, fill: 1),
                          ),
                          label: _tabSpecs[routeIndex].labelKey.tr(),
                        ),
                      NavigationDestination(
                  ...
                  for (final (position, routeIndex) in order.indexed)
                    ReorderableDragStartListener(
                      ...
                      child: _RailDestination(
                        spec: _tabSpecs[routeIndex],
                        selected: activeIndex == routeIndex,
                        showBadge:
                            _tabSpecs[routeIndex].badged && githubHasFailures,
            ...
            child: Column(
              ...
              children: [
                ...
                IconButton(
                  ...
                  onPressed: () => onSelected(6),
                  icon: Icon(Symbols.settings, fill: activeIndex == 6 ? 1 : 0),
                ),
              ],
            ),
          ),

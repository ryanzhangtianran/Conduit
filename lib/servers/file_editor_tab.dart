...
class _FileEditorTabViewState extends ConsumerState<FileEditorTabView> {
  ...
  bool get _canFormat =>
      _kind != null &&
      canFormatStructuredDocument(_kind!) &&
      !_loading &&
      !_saving.value;
  ...
  void initState() {
    ...
    ref
        ...
        .registerCloseGuard(widget.tab.id, _requestClose);
    unawaited(_load());
  }

  @override
  void dispose() {
    ref.read(terminalTabsProvider.notifier).unregisterCloseGuard(widget.tab.id);
    // Each editor opens its own SFTP channel; release it with the tab.
    final sftp = _sftpClient;
    _sftpClient = null;
    if (sftp != null) {
      unawaited(sftp.then((client) => client.close()).catchError((_) {}));
    }
    if (_listening) {

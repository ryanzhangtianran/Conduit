...
class _SessionsWorkspaceState extends ConsumerState<SessionsWorkspace> {
  /// Shows the server grid even while sessions stay open in the background.
  var _showHome = false;

  @override
  Widget build(BuildContext context) {
    // A newly opened or selected session takes over from the home grid.
    ref.listen(terminalTabsProvider.select((tabs) => tabs.selectedId), (
      previous,
      next,
    ) {
      if (next != null && next != previous && _showHome) {
    ...
    final showHome = _showHome || tabs.isEmpty;
    final selectedTab = tabs.selectedTab;
    final focusedTerminal = !showHome && selectedTab is TerminalTab
        ? selectedTab
        : null;
    final focusedSession = showHome
        ? null
        : _sessionForTab(sessions, selectedTab);
              ...
              child: Row(
                ...
                children: [
                  _SessionSidebar(
                    ...
                    onShowHome: () => setState(() => _showHome = true),
                    onSelectTab: (tab) {
                      setState(() => _showHome = false);
                      final notifier = ref.read(terminalTabsProvider.notifier);
                      final paneId = tabs.paneIdForTab(tab.id);
                      if (paneId != null) notifier.focusPane(paneId);
                      ...
                      children: [
                        for (final tab in tabs.tabs)
                          _AnimatedPaneTabContent(
                            key: ValueKey(tab.id),
                            active: !showHome && tab.id == selectedTab?.id,
                            child: _SessionTabBody(
                              tab: tab,
                              autofocus: !showHome && tab.id == selectedTab?.id,
                            ),
                          ),
                        if (showHome)
                          ColoredBox(

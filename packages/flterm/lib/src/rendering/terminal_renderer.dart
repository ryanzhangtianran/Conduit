...
class TerminalRenderBox extends RenderBox {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    canvas.save();
    // Land the glyph grid on whole device pixels: when this repaint
    // boundary is composited at a fractional device offset, every atlas
    // texel is resampled and the whole terminal looks soft. The correction
    // shifts content by less than half a device pixel.
    ...
    if (attached && dpr > 0) {
      ...
      if (origin.dx.isFinite && origin.dy.isFinite) {
        dx += (origin.dx * dpr).roundToDouble() / dpr - origin.dx;
        dy += (origin.dy * dpr).roundToDouble() / dpr - origin.dy;
      }
    }

...
class TrayManager {
  ...
  Future<void> setContextMenu(Menu menu) async {
    ...
    if (defaultTargetPlatform == TargetPlatform.macOS) {
      await _inlineMenuIcons(json);
    }
    final Map<String, dynamic> arguments = {'menu': json};
    await _channel.invokeMethod('setContextMenu', arguments);
  }

  /// Conduit addition: a menu item's `icon` (a Flutter asset path) is sent
  /// to macOS as base64 image data under `iconBase64`, so items can carry
  /// small native images (status dots) next to their labels.
  Future<void> _inlineMenuIcons(Map<String, dynamic> menuJson) async {
    final items = menuJson['items'];
    ...
    for (final item in items) {
      ...
      if (icon is String && icon.isNotEmpty) {
        try {
          ...
          item['iconBase64'] = base64Encode(data.buffer.asUint8List());
        } catch (_) {
          // Missing asset: the item simply renders without an image.
        }
      }
      final submenu = item['submenu'];
      if (submenu is Map<String, dynamic>) await _inlineMenuIcons(submenu);
    }
  }

...
void main() {
  ...
  group('ConnectionImportService', () {
    ...
    test('servers sharing a credential are re-linked to one row', () async {
      // Two servers on one saved credential, as the former credential picker
      // produced.
      final first = await repository.create(
        ServerDraft(
          ...
          port: 22,
          username: 'ops',
          credential: ServerCredential.password('shared-pass'),
          credentialName: 'ops-cred',
        ),
      );
      await repository.create(
        ServerDraft(
      ...
      final export = await exportService.exportJson(passphrase: 'export-pass');
      final document = jsonDecode(export) as Map<String, dynamic>;
      for (final record in document['servers'] as List) {
        expect(record['credentialIndex'], 0);
      }
      ...
      for (final candidate in candidates) {
        expect(candidate.connection.sharedCredentialIndex, 0);
        expect(candidate.connection.credentialName, 'ops-cred');
        expect(candidate.connection.credential?.password, 'shared-pass');
      }

      // Import into a clean table so the created rows are easy to count.
      for (final server in await repository.all()) {
        await repository.delete(server);
      }
      final before =
          (await database.select(database.savedCredentials).get()).length;
      await importService.import(candidates);
      final imported = await repository.all();
      expect(imported, hasLength(2));
      expect(imported[0].credentialId, imported[1].credentialId);
      final rows = await database.select(database.savedCredentials).get();
      expect(rows.length, before + 1);
      expect(rows.last.name, 'ops-cred');
      expect(
        (await repository.credentialFor(imported[1])).password,
        'shared-pass',
      );
    });

    test('documents without credentialIndex get private credentials', () async {
      final document = {
        'format': ConnectionExportService.formatName,
        'version': 1,
        'servers': [
          {'name': 'a', 'host': 'a.local', 'port': 22, 'username': 'u'},
          {'name': 'b', 'host': 'b.local', 'port': 22, 'username': 'u'},
        ],
        'savedCredentials': [
          {'name': 'shared', 'authType': 'password'},
        ],
        'secrets': await vault.encryptPortable(
          jsonEncode({
            'servers': [
              {
                'index': 0,
                'credential': ServerCredential.password('p').toJson(),
              },
              {
                'index': 1,
                'credential': ServerCredential.password('p').toJson(),
      ...
      expect(candidates.map((c) => c.connection.sharedCredentialIndex), [
        null,
        null,
      ]);
      await importService.import(candidates);
      final imported = await repository.all();
      expect(imported[0].credentialId, isNot(imported[1].credentialId));
    });

    test('a document version newer than the app is rejected', () async {
      await expectLater(
        importService.previewJson(
          jsonEncode({
            'format': ConnectionExportService.formatName,
            'version': ConnectionExportService.formatVersion + 1,
            'servers': <Object>[],
          }),
        ),

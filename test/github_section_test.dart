...
void main() {
        ...
        ProviderScope(
          ...
          overrides: [
            githubRunsProvider.overrideWith(
              (ref) => Stream.value(GitHubRunsSnapshot.empty),
            ),
          ],
          child: const MaterialApp(
            home: Scaffold(body: GithubWorkflowStatusStrip()),
  ...
  testWidgets('a rate-limited snapshot keeps the strip and explains it', (
    WidgetTester tester,
  ) async {
      ...
      EasyLocalization(
        ...
        child: ProviderScope(
          overrides: [
            githubRunsProvider.overrideWith(
              (ref) =>
                  Stream.value(snapshotWithFailures().withRateLimit(resetAt)),
            ),
          ],
          child: MaterialApp(
            locale: Locale('en', 'US'),
    ...
    await tester.pumpAndSettle();
    // The last known run is still shown rather than the strip disappearing.
    expect(find.text('Build'), findsOneWidget);
    // The header carries the rate-limit hint (an hourglass with a tooltip).
    expect(find.byType(Tooltip), findsOneWidget);
  });

  testWidgets('a connection without a token renders as signed out', (
    WidgetTester tester,
  ) async {
    final connection = GitHubConnection(
      id: 1,
      ...
      EasyLocalization(
        ...
        child: ProviderScope(
          overrides: [
            ...
            // The token was revoked (401) or never restored from a backup.
            githubTokenForConnectionProvider.overrideWith((ref) async => null),
          ],
          child: MaterialApp(
            locale: Locale('en', 'US'),
            home: Scaffold(body: ListView(children: const [GitHubSection()])),

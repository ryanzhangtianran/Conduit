...
''';
  ...
  group('reorderSshConfigHosts', () {
    ...
    test('moves blocks while fixed material keeps its slot', () {
      final result = reorderSshConfigHosts(config, ['c', 'a', 'b']);
      expect(
        result,
        '# global\n'
        'ServerAliveInterval 30\n'
      ...
      expect(parseSshConfigHosts(result).map((h) => h.alias), ['c', 'a', 'b']);
    });

    test('unknown aliases are ignored and unlisted blocks trail', () {
      final result = reorderSshConfigHosts(config, ['zzz', 'b']);
      expect(parseSshConfigHosts(result).map((h) => h.alias), ['b', 'a', 'c']);
    });

    test('is a no-op for fewer than two blocks or an unchanged order', () {
      expect(
        reorderSshConfigHosts('Host a\n  HostName 1\n', ['a']),
        'Host a\n  HostName 1\n',
      );
      expect(reorderSshConfigHosts('', ['a']), '');
    });
  });
}
